...
futures_tickers = {
    "貴金属": [
        ...
        "SI=F  # Silver (銀)",
        "PL=F  # Platinum (プラチナ)",
        "PA=F  # Palladium (パラジウム)",
    ],
    "産業金属": [
        "HG=F  # Copper (銅)",
    ],
    "エネルギー": [
        "CL=F  # Crude Oil WTI (WTI原油)",
        "BZ=F  # Brent Crude Oil (ブレント原油)",
        "NG=F  # Natural Gas (天然ガス)",
        "HO=F  # Heating Oil (ヒーティングオイル)",
        "RB=F  # RBOB Gasoline (ガソリン)",
    ],
    "穀物": [
        "ZC=F  # Corn (とうもろこし)",
        "ZS=F  # Soybeans (大豆)",
...
try:
    ...
    if results:
        ...
        # 6. 結果をCSVファイルに保存
        file_path = os.path.join(save_folder, "top_30_commodity_by_growth.csv")
...
try:
    ...
    candidate_files = [
        ...
        if (f == "top_30_commodity_by_growth.csv" or  # 元のCSVファイル
           (f.startswith("_top30_") and f.endswith(".xlsx"))) # 新しいExcelファイル
    ]
    
    if not candidate_files:
        raise FileNotFoundError(f"分析フォルダ内に 'top_30_commodity_by_growth.csv' または 'etf_top30_*.xlsx' ファイルが見つかりません。")
...
try:
    ...
    output_path = os.path.join(folder, f"commodity_ranked_top_{len(top_ranked)}_{timestamp}.xlsx")
