...
simulation_results = []
initial_investment = 2000000; leverage = 3; stop_loss_threshold = 0.3
...
# ==============================================================================
# 6. 再投資シミュレーションと結果分析 (価格データがNaNの場合はスキップする修正済み)
# ==============================================================================
...
if not df_simulation.empty:
    # NaNチェックのために 'Start Price' と 'Exit Price' を数値型に変換しておく
    df_simulation['Start Price'] = pd.to_numeric(df_simulation['Start Price'], errors='coerce')
    df_simulation['Exit Price'] = pd.to_numeric(df_simulation['Exit Price'], errors='coerce')
    ...
    for sim_date in simulation_dates:
        ...
        for holding in current_holdings:
            # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
            # 修正箇所：価格データがNaNの場合はスキップする処理を追加
            # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
            row = holding["row_data"]
            
            # 1. 売却タイミングのチェック
            if sim_date >= (holding["enter_date"] + pd.DateOffset(days=deal_term)).date():
                
                # 2. 価格データがNaNでないかをチェック
                if pd.isna(row["Exit Price"]) or pd.isna(row["Start Price"]):
                    print(f"[{sim_date}] スキップ (価格データNaN): {row['Ticker']}")
                    # 価格がNaNの場合、この取引は勘定せずにそのまま保有リストから削除する
                    # total_tradesにも加算しない
                    continue # 次のholdingのループへ

                # 3. 価格データが有効な場合のみ、通常の売却処理を行う
                total_trades += 1
                ...
                # 売却タイミングでない場合は、そのまま次の保有リストへ
                holdings_after_sell.append(holding)
            # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
            # 修正ここまで
            # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
        ...
        if available_slots > 0:
            ...
            # 購入候補からも価格がNaNのものは除外する
            df_deals = df_deals.dropna(subset=['Start Price', 'Exit Price'])
    ...
    # --- (ここから下のファイル保存処理も変更なし) ---
    if not df_portfolio_progression.empty:
        trade_log_file_name = f"./分析/trade_log_max{MAX_HOLDINGS}_ensemble_5models_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    ...
    weekday_stats = df_simulation.dropna(subset=['Profit/Loss (JPY)']).groupby("Weekday").apply(lambda x: pd.Series({"勝利数": (x["Profit/Loss (JPY)"] > 0).sum(), "取引数": len(x), "勝率": ((x["Profit/Loss (JPY)"] > 0).sum() / len(x)) if len(x) > 0 else 0})).reset_index()
    weekday_stats["曜日"] = weekday_stats["Weekday"].map({0: "月", 1: "火", 2: "水", 3: "木", 4: "金", 5: "土", 6: "日"})
    weekday_winrates_output_file = f"./分析/weekday_winrates_ensemble_5models_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    weekday_stats[["曜日", "勝利数", "取引数", "勝率"]].to_excel(weekday_winrates_output_file, index=False)
    
    ticker_stats = df_simulation.dropna(subset=['Profit/Loss (JPY)']).groupby("Ticker").apply(lambda x: pd.Series({"勝利数": (x["Profit/Loss (JPY)"] > 0).sum(), "取引数": len(x), "勝率": ((x["Profit/Loss (JPY)"] > 0).sum() / len(x)) if len(x) > 0 else 0})).reset_index()
    ticker_stats["Company Name"] = ticker_stats["Ticker"].apply(get_company_name)
    ticker_winrates_output_file = f"./分析/ticker_winrates_ensemble_5models_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
# 送信前に数値をクリーニング
if 'current_portfolio' not in locals() or math.isnan(current_portfolio):
    current_portfolio = 0
if 'initial_investment' not in locals() or math.isnan(initial_investment):
    initial_investment = 0

# =================================================================
# ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼ このコードで既存のtry-exceptを置き換えてください ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
# =================================================================
from email.header import Header # ★★★★★ 1. Headerをインポート
import traceback # エラー詳細表示のため

try:

    # ★★★★★ ここが最重要の修正点です ★★★★★
    # コンピュータ名をASCII互換の 'localhost' に指定して接続を開始します
    ...
    # ゼロ除算を回避
    raito = current_portfolio / initial_investment if initial_investment > 0 else 0
    profit = current_portfolio - initial_investment

    # --- HTMLテーブル作成処理 (この部分は元のコードのまま) ---
    ...
    if not df_portfolio_progression.empty:
        ...
        simulation_results_html = df_portfolio_progression.to_html(index=False, justify="center", float_format="{:.2f}".format, border=1, escape=False)
    else:
    ...
    for recipient in recipient_list:
        ...
        # ★★★★★ 2. 件名をHeaderオブジェクトでエンコード
        subject = f"商品v3 アンサブル5 1DAY TOP40 アンサンブル推奨 ({current_date}) {int(current_portfolio)}円 {raito:.2f}倍"
        msg["Subject"] = Header(subject, 'utf-8')

        # ★★★★★ 3. 本文(HTML)の文字コードを 'utf-8' に明示的に指定
        body_html = f"""
        <html>
        <head>
            <meta charset="UTF-8">
        ...
        # ★★★★★ 4. 添付ファイル名のエンコードを正しく処理 (前回の修正を適用)
    ...
    print(f"送信エラー: {e}")
    traceback.print_exc() # エラーの発生箇所を詳しく表示