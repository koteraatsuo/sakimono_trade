...
def create_features_and_labels(periods_input):
    ...
    for period_ in periods_input:
        for ticker_ in ticker_symbols:
            ...
            try:
                # Close価格の時系列を取得
                series = ticker_data_dict[ticker_]['Close']
    ...
    for idx, row in top_stocks_sim.iterrows():
        ...
        try:
            ...
            if end_idx_ >= len(df_ticker):
                continue
            start_price = df_ticker['Close'].iloc[start_idx_]
            end_price = df_ticker['Close'].iloc[end_idx_]
            low_prices = df_ticker['Low'].iloc[start_idx_+1:end_idx_+1]
