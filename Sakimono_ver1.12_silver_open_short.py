...
try:
    ...
    for recipient in recipient_list:
        ...
        msg["To"] = recipient
        if entry_ok :
            msg["Subject"] = f"SILVER OK {probability*100:.1f}%実績のあるGOLD購入リストのおすすめ結果 ({current_date}) {int(investment)}円 {raito:.2f}倍"
        else:
            msg["Subject"] = f"SILVER NG {probability*100:.1f}%実績のあるGOLD購入リストのおすすめ結果 ({current_date}) {int(investment)}円 {raito:.2f}倍"
