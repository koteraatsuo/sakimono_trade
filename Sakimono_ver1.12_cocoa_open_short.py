...
# for num in np.arange(0.007, 0.100, 0.001):
# for num in np.arange(0.05, 1, 0.05):
# for num in np.arange(1, 40, 1):
print(data.tail())
initial_investment = 100000
leverage = 1
stop_loss_threshold = 0.017
...
def get_daily_close_from_hourly(ticker, period="10y"):
    ...
    print("="*50)
    print("当日分まとめる前")
    print(df_hourly.tail(50))
    print("="*50)
    df_hourly = df_hourly.sort_index()  # 念のため時系列ソート
    df_daily = df_hourly['Open'].resample('D').first().dropna()
    print("当日分まとめる後")
    print(df_daily.tail(25))
    print("="*50)
...
if float(recommendation_data.head(1)["Probability"].iloc[0]) < Probability_threshold:
    entry_ok = False
else:
    entry_ok = True
probability = recommendation_data.head(1)["Probability"].iloc[0]
...
for top_n, purchase_recommendations in [(3, purchase_recommendations_top3), (5, purchase_recommendations_top5)]:
    ...
    for idx, row in top_stocks_rec.iterrows():
        ...
        company_name = tickers.get(ticker_, "")
        # try:
        df_temp = yf.download(ticker_, period="5d", interval="1h", progress=False)["Open"].dropna()
...
def generate_advanced_chart(ticker, save_path, period="5d", interval="15m"):
    ...
    try:
        ...
        mpf.plot(
            ...
            type="candle",
            style="yahoo",
            title=f"{ticker} Advanced Chart ({interval}足)",
            ylabel="Price",
            volume=True,
            addplot=add_plots,
            savefig=save_path,
...
try:
    ...
    for recipient in recipient_list:
        ...
        msg["To"] = recipient
        if entry_ok :
            msg["Subject"] = f"ココア OK {probability*100:.1f}%実績のある購入リストのおすすめ結果 ({current_date}) {int(investment)}円 {raito:.2f}倍"
        else:
            msg["Subject"] = f"ココア NG {probability*100:.1f}%実績のある購入リストのおすすめ結果 ({current_date}) {int(investment)}円 {raito:.2f}倍"
